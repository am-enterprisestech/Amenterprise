...
export const updateClientCredentials = createServerFn({
  method: "POST",
})
  .middleware([requireSupabaseAuth])
  .validator(
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.password) {
      patch.password = cleanPassword(data.password);
    }

    if (data.email) {
      patch.email = cleanEmail(data.email);
    }
    ...
    if (client.user_id) {
      ...
      throw new Error(
        "This client has no login yet — set both email and password."
      );
    }
...
export const deleteClientAccount = createServerFn({
  method: "POST",
})
  .middleware([requireSupabaseAuth])
  .validator(
